...
async function sweepFee(){
  ...
  if (window.AUREUM_DEPLOYMENT && window.aureumLiveSweep) {
    if (s) { s.textContent = 'sweeping onchain…'; s.style.color = 'var(--gold)'; }
    try {
      var ok = await window.aureumLiveSweep();
      if (ok) { if (s) { s.textContent = 'funded ✓ onchain'; s.style.color = 'var(--gold-bright)'; } setTimeout(function(){ if (s) { s.textContent = 'idle'; s.style.color = 'var(--gold)'; } }, 1600); return; }
    } catch (e) { console.warn('Aureum: live sweep failed, falling back to illustrative math', e); }
  }
  ...
  if (s) { s.textContent = 'funded ✓'; s.style.color = 'var(--gold-bright)'; setTimeout(function(){ s.textContent = 'idle'; s.style.color = 'var(--gold)'; }, 1200); }
}
var spentEth = 0.22;
