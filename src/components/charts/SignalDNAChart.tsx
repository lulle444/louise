...
export function SignalDNAChart({ data, accent = "#21D4FD" }: { data: Array<{ axis: string; value: number }>; accent?: string }) {
  const summary = data.map((d) => `${d.axis} ${d.value}`).join(", ");
  return (
    <div className="h-64 sm:h-72" role="img" aria-label={`Signal DNA radar. ${summary}.`}>
