...
// creature party, dex registry, player level & perks, achievement
...
export class Inventory {
  constructor() {
    ...
    this.milestone = 0;        // highest dex milestone reward claimed
  ...
  // ---- party / dex ----
  capture(creature) {
    this.dexCaught.add(creature.key);
    this.dexSeen.add(creature.key);
  ...
  see(key) { if (!this.dexSeen.has(key)) { this.dexSeen.add(key); this.save(); } }
  ...
  starter(key) {
    this.dexCaught.add(key); this.dexSeen.add(key);
  ...
  evolve(i) {
    ...
    this.dexCaught.add(to); this.dexSeen.add(to);
    this.save();
    return { from, to };
  }
  ...
  checkAchievements(extra = {}) {
    const merged = { ...this.ach, ...extra, species: this.dexCaught.size, goldEarned: this.goldEarned, plevel: this.plevel };
    const out = [];
  ...
  save() {
    try {
      localStorage.setItem(SAVE_KEY, JSON.stringify({
        ...
        seen: [...this.dexSeen], caught: [...this.dexCaught], milestone: this.milestone,
  ...
  load() {
    try {
      ...
      this.dexSeen = new Set(d.seen || []);
      this.dexCaught = new Set(d.caught || []);
