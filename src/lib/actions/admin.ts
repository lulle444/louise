...
function revalidateAll(battleId?: string) {
  ...
  if (battleId) revalidatePath(`/rounds/${battleId}`);
}
...
export async function createBattleAction(_prev: AdminActionState, formData: FormData): Promise<AdminActionState> {
  ...
  try {
    ...
    return { ok: true, message: `${battle.status === "draft" ? "Draft created" : "Round published"}: ${battle.title} (${battle.slug}).` };
  } catch (err) {
    return { ok: false, message: err instanceof Error ? err.message : "Could not create Round." };
  }
}
...
export async function transitionBattleAction(_prev: AdminActionState, formData: FormData): Promise<AdminActionState> {
  ...
  try {
    ...
    if (!battle) return { ok: false, message: "Round not found" };
    switch (action) {
      case "publish": {
      ...
      case "lock": {
        if (!["upcoming", "open"].includes(battle.status)) return { ok: false, message: `Cannot lock a ${battle.status} Round.` };
        const now = new Date().toISOString();
        await repo.updateBattle(battleId, { status: "locked", locksAt: now });
        await repo.appendAudit({ actorId: viewer.id, action: "battle.lock", targetType: "round", targetId: battleId, details: { locksAt: now, reason: reason ?? null } });
        revalidateAll(battleId);
        return { ok: true, message: "Round locked early; no further calls accepted." };
      }
      case "settle": {
      ...
      case "void": {
        ...
        revalidateAll(battleId);
        return { ok: true, message: "Round voided. Calls are excluded from accuracy." };
      }
      case "archive": {
        if (!["settled", "void", "draft"].includes(battle.status)) return { ok: false, message: "Only settled, void or draft Rounds can be archived." };
        await repo.updateBattle(battleId, { status: "archived" });
        await repo.appendAudit({ actorId: viewer.id, action: "battle.archive", targetType: "round", targetId: battleId, details: { previous: battle.status } });
        revalidateAll(battleId);
        return { ok: true, message: "Round archived." };
      }
      case "unarchive": {
        if (battle.status !== "archived") return { ok: false, message: "Round is not archived." };
        const restored = battle.outcome ? "settled" : "void";
        await repo.updateBattle(battleId, { status: restored });
        await repo.appendAudit({ actorId: viewer.id, action: "battle.unarchive", targetType: "round", targetId: battleId, details: { restored } });
        revalidateAll(battleId);
        return { ok: true, message: `Round restored as ${restored}.` };
      }
    }
  } catch (err) {
...
export async function toggleAIProfileAction(_prev: AdminActionState, formData: FormData): Promise<AdminActionState> {
  ...
  try {
    ...
    if (!parsed.data.enabled && existing.length > 0) return { ok: false, message: "This profile has already locked a forecast for the Round and cannot be removed." };
    ...
    await repo.appendAudit({ actorId: viewer.id, action: "battle.ai_toggle", targetType: "round", targetId: battle.id, details: { aiProfileId: parsed.data.aiProfileId, enabled: parsed.data.enabled } });
    revalidateAll(battle.id);
