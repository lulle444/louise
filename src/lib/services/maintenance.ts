...
const g = globalThis as unknown as { __alphrMaintenanceAt?: number; __alphrMaintenanceRunning?: boolean; __alphrMaintenanceStatus?: MaintenanceStatus };

export function getMaintenanceStatus(): MaintenanceStatus {
  return g.__alphrMaintenanceStatus ?? { lastRunAt: null, lastSource: null, lastReport: null, lastError: null };
}
...
export async function runMaintenanceNow(source: string, actorId: string | null = null): Promise<CronReport> {
  ...
  try {
    ...
    g.__alphrMaintenanceStatus = { lastRunAt: new Date().toISOString(), lastSource: source, lastReport: report, lastError: null };
    g.__alphrMaintenanceAt = Date.now();
    return report;
  } catch (err) {
    const message = err instanceof Error ? err.message : String(err);
    g.__alphrMaintenanceStatus = { lastRunAt: new Date().toISOString(), lastSource: source, lastReport: null, lastError: message };
    throw err;
  }
}
...
export async function maybeRunMaintenance(): Promise<void> {
  ...
  const now = Date.now();
  if (g.__alphrMaintenanceRunning) return;
  if (g.__alphrMaintenanceAt && now - g.__alphrMaintenanceAt < THROTTLE_MS) return;
  g.__alphrMaintenanceRunning = true;
  g.__alphrMaintenanceAt = now;
  try {
    await runMaintenanceNow("page-view");
  } catch (err) {
    console.error("[maintenance]", err instanceof Error ? err.message : err);
  } finally {
    g.__alphrMaintenanceRunning = false;
  }
}
