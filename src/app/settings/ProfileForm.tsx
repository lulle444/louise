  ...
  return (
    <form action={action} className="card space-y-5 p-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <button type="submit" disabled={pending} className="rounded-full bg-cyan px-4 py-2 text-sm font-semibold text-white hover:brightness-110 disabled:opacity-60">{pending ? "Saving…" : "Save changes"}</button>
      </div>
    </form>
  );
}
