...
export default async function HumansVsAiPage() {
  ...
  return (
    <>
      <PageHeader eyebrow="Humans vs AI" title="Who reads crypto markets best?" description={<>Human analysts, three simulated AI profiles and the crowd forecast the same Battles. Everything below is derived from settled results. <Link href="/methodology" className="text-cyan hover:underline">How scoring works →</Link></>} />
      <div className="mx-auto max-w-7xl space-y-10 px-4 py-8 sm:px-6">
        <Scoreboard summary={s} />

        <div className="grid gap-4 sm:grid-cols-3">
          <StatCard label="Battles decided" value={s.battleWins.humans + s.battleWins.ai + s.battleWins.ties} hint={`Humans ${s.battleWins.humans} · AI ${s.battleWins.ai} · Ties ${s.battleWins.ties}`} />
        ...
        <section className="card p-5" aria-labelledby="trend-heading">
          ...
          <p className="mb-4 text-sm text-muted">Share of correct forecasts per settled Battle, humans vs the three AI profiles.</p>
          <TrendChart data={s.trend} />
        </section>

        <Section eyebrow="AI analysts" title="Three profiles, three styles" description="These are deterministic rule-based simulations built for the Arena. They are not commercial AI models and their forecasts are not financial advice." className="!px-0 !py-0">
        ...
        <div className="grid gap-6 lg:grid-cols-[1.3fr_1fr]">
          <section className="card overflow-hidden" aria-labelledby="recent-heading">
            <h2 id="recent-heading" className="border-b border-border px-5 py-4 text-base font-semibold">Recent Battles</h2>
            {s.recentBattles.length === 0 ? <div className="p-5"><EmptyState title="No settled Battles yet" /></div> : (
              <ul className="divide-y divide-border">
                {s.recentBattles.map((r) => (
                  <li key={r.battle.id} className="flex flex-wrap items-center gap-3 px-5 py-3 text-sm">
                    ...
                    <Link href={`/arena/${r.battle.id}`} className="min-w-0 flex-1 hover:underline">
          ...
          <section className="card p-5" aria-labelledby="disagree-heading">
            ...
            {s.biggestDisagreement ? (
              <div className="mt-3 text-sm">
                <Link href={`/arena/${s.biggestDisagreement.battle.id}`} className="font-semibold hover:underline">{s.biggestDisagreement.battle.title}</Link>
            ...
            ) : <p className="mt-3 text-sm text-muted">Humans and AI have agreed on every settled Battle so far.</p>}
