...
export const metadata: Metadata = { title: "Leaderboard", description: "Arena ratings for ranked analysts by accuracy, experience and consistency." };
...
export default async function LeaderboardPage(props: PageProps<"/leaderboard">) {
  ...
  return (
    <>
      <PageHeader eyebrow="Leaderboard" title="Arena ratings" description={<>Only analysts with at least {MIN_RANKED_BATTLES} valid settled Battles in the selected scope are ranked. Rating = accuracy × 0.6 + experience (up to 25) + consistency (up to 15). <Link href="/methodology#leaderboard" className="text-cyan hover:underline">Formula →</Link></>} />
      <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6">
        ...
        <div className="mt-6">
          ...
          <p className="mt-3 text-xs text-muted">
            {ranked.length} ranked · {unranked.length} provisional (fewer than {MIN_RANKED_BATTLES} settled Battles in scope; shown unranked with a provisional rating). Track records are forecasting-game history, not investment performance.
          </p>
        </div>
      </div>
    </>
