      ...
      <div className="mx-auto grid max-w-7xl gap-10 px-4 py-8 sm:px-6 lg:grid-cols-[14rem_1fr]">
        ...
        <div className="prose-arena max-w-3xl space-y-10 text-sm leading-relaxed text-muted [&_code]:rounded [&_code]:bg-surface-2 [&_code]:px-1.5 [&_code]:py-0.5 [&_code]:font-mono [&_code]:text-[12px] [&_code]:text-text [&_li]:mt-1 [&_p]:mt-2 [&_ul]:list-disc [&_ul]:pl-5">
          ...
          <section>
            ...
            <ul>
              <li><strong className="text-text">Start price</strong> is captured when the Round opens (<code>opens_at</code>), from the provider’s historical price closest to that instant.</li>
              <li><strong className="text-text">Reference price</strong> is stored on every call at lock time for transparency; it does not affect scoring.</li>
          ...
          <section>
            ...
            <p>Each Round stores a <code>neutral_threshold_percent</code> (default {DEFAULT_NEUTRAL_THRESHOLD_PERCENT}% for a 24-hour Round).</p>
...
Neutral  otherwise (a change exactly equal to ±threshold is Neutral)`}</pre>
          ...
          <section>
            ...
            <ul>
              <li>Calls are accepted only while <code>opens_at ≤ now &lt; locks_at</code>. Deadlines are enforced on the server and in the database, never just by the countdown.</li>
              <li>A scheduled job (and the admin console) settles Rounds once <code>now ≥ ends_at</code>. The Round moves to <code>settling</code>, which acts as a lock against concurrent runs, then to <code>settled</code>.</li>
              <li>Settlement is idempotent: a settled Round is never settled twice, and XP is written through a ledger keyed by (user, Round, reason) so replays cannot double-award.</li>
              <li>Correct direction = 100 Round Score. Incorrect = 0. Confidence is recorded for calibration but does not multiply the score.</li>
            </ul>
          </section>
          <section>
            <H id="void">Void rules</H>
            <p>If required price data is unavailable or clearly invalid after retries, an administrator may void the Round. Void Rounds award no score and no XP, do not affect streaks, and are excluded from accuracy, ratings and leaderboards. Settled Rounds cannot be voided; they can only be archived.</p>
          </section>
          <section>
          ...
          <section>
            ...
            <p>The Crowd call is the percentage of locked <em>human</em> calls by direction. AI forecasts are never included. Exact percentages are revealed to a viewer only after they lock their own call or once the Round is no longer accepting calls; visitors see an obscured teaser.</p>
          </section>
          <section>
            ...
            <p>Open, pending and void calls are excluded. The 7-day and 30-day views filter by the Round’s end time.</p>
          </section>
          <section>
...
arena_rating = round(sum, 1)                                  (max 100)`}</pre>
            <p>Analysts need at least {MIN_RANKED_BATTLES} valid settled Rounds in the selected scope to be ranked. Below that they appear unranked with a provisional rating. Ties are broken by accuracy, then by settled count.</p>
          </section>
          <section>
            <H id="xp">XP, levels and badges</H>
            <ul>
              <li>Lock a valid call: +{XP_RULES.lock} XP · Correct call: +{XP_RULES.correct} XP · Three-correct streak: +{XP_RULES.streak_3} XP · Five-correct streak: +{XP_RULES.streak_5} XP · Seven valid Daily Rounds completed: +{XP_RULES.seven_battles} XP (once).</li>
          ...
          <section>
            <H id="dna">Call Profile</H>
            <p>Call Profile summarises a user’s forecasting-game history: most-used signal, best-performing signal (minimum {MIN_SIGNAL_USES} settled uses), best asset (minimum {MIN_ASSET_BATTLES} Rounds), accuracy by direction, confidence calibration and a market-style label. It is computed only once a user has {MIN_DNA_BATTLES} valid settled Rounds; before that the profile shows “Building your Call Profile”. Call Profile is not evidence of trading profitability.</p>
