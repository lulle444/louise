// ============================================================
// WANDERMERE — 2D top-down survival (Phaser 3). A procedural
// world of biomes, original magical familiars (procedurally drawn
// — see sprites.js) that wander and can be weakened & tamed, plus
// survival, resources, crafting, building, day/night and weather.
// ============================================================

import { SPECIES, ITEMS, BUILDINGS, SEA, BUFFS, FISH, typeMult, TYPE_COLORS, BOSS, BOSS_POOL, EGG, ivHpMult } from './data.js?v=51';
import { ensureCreatureTexture } from './sprites.js?v=51';
...
const QUESTS = [
  ...
  { title: 'Complete the Bestiary (all species)', max: 149, stat: 'species', reward: { evostone: 12, masterball: 2 } },
];
...
const LEGEND_LOG_KEY = 'wandermere_legend_log';
...
// Generate every creature's texture in ONE up-front batch after create() so
// the world never has to draw a species mid-frame later; procedural canvas
// generation is cheap and synchronous, so (unlike the old streamed-in art)
// everything is ready immediately — this just pre-warms the per-species
// cache in sprites.js and makes sure anything already on screen is showing
// its real texture rather than a stale one.
function loadAllSprites(scene, creatures) {
  for (const [key, sp] of Object.entries(SPECIES)) ensureCreatureTexture(scene, key, sp);
  for (const c of creatures) {
    const texKey = 'mon_' + c.key;
    if (scene.textures.exists(texKey) && c.spr.texture.key !== texKey) {
      c.spr.setTexture(texKey).setScale(c.sp.scale * 0.55).setAlpha(1).setOrigin(0.5, footFrac(scene, texKey));
    }
  }
}
...
export function startGame(options = {}) {
  ...
  if (MODE === 'multiplayer') SEED = strSeed(ROOM);
    ...
    try { let s = +localStorage.getItem('wandermere_seed'); if (!s) { s = (Math.random() * 1e9) | 0; localStorage.setItem('wandermere_seed', String(s)); } SEED = s; } catch (e) {}
  }
  ...
  function create() {
    ...
    scene.input.keyboard.on('keydown-P', () => ui.toggle('bestiary'));
    ...
    const stats = { wood: 0, stone: 0, caught: 0, crafted: 0, built: 0, tools: 0, farms: 0, evolved: 0, days: 1, fished: 0, cooked: 0, defeated: 0, bosses: 0, species: inv.bestiaryCaught.size };
    ...
    ui.chat('Welcome to Wandermere — tame familiars, survive, and build with friends.', 'sys');
    ...
    function spawnFollower(member) {
      ...
      const texKey = ensureCreatureTexture(scene, member.key, sp);
      const spr = scene.add.image(player.x, player.y, texKey).setScale(sp.scale * 0.4).setDepth(player.y);
      spr.setOrigin(0.5, footFrac(scene, texKey));
      if (member.gloam) spr.setTint(0xc9a0ff); else if (member.shiny) spr.setTint(0xfff3a0);
      followers.push({ spr, member, key: texKey });
    ...
    function awardXp(n) {
      ...
      if (r) {
        ...
        if (r.evolved) {
          ...
          S.state.stats.species = inv.bestiaryCaught.size; bumpStat('species', 0); bestiaryMilestone(); rebuildFollowers();
    ...
    // long-term bestiary milestones — reward taming more unique species
    const MILESTONES = [[5, { evostone: 1 }], [10, { ultraball: 3 }], [15, { evostone: 2 }], [20, { ultraball: 5 }], [30, { evostone: 4 }], [42, { evostone: 10 }]];
    function bestiaryMilestone() {
      const n = inv.bestiaryCaught.size;
      for (const [need, rew] of MILESTONES) {
        if (n >= need && inv.milestone < need) {
          ...
          ui.chat(`★ Bestiary milestone — ${need} species registered! +${rs}`, 'evt');
          ui.toast(`Bestiary ${need} species! +${rs}`, '#ffd24a'); SFX.levelup();
          ui.refreshHotbar(); ui.updateResources();
        }
      }
    }
    scene._bestiaryMilestone = bestiaryMilestone;
    ...
    ui.onEvolve = (r) => {
      ...
      S.state.stats.species = inv.bestiaryCaught.size; bumpStat('species', 0); bestiaryMilestone();
      const pf = scene.add.particles(player.x, player.y - 16, 'flake', { speed: { min: 40, max: 140 }, lifespan: 600, scale: { start: 1.6, end: 0 }, quantity: 18, tint: 0xb088ff, emitting: false }).setDepth(player.y + 60);
      pf.explode(18); scene.time.delayedCall(700, () => pf.destroy());
    };
    ...
    function placeBuild() {
      const def = BUILDINGS[build.type];
      if (def.unlock && inv.bestiaryCaught.size < def.unlock) { ui.toast(`${def.name} locked — register ${def.unlock} species.`, '#ff8787'); return; }
    ...
    const showTutorialOnce = () => {
      if (localStorage.getItem('wandermere_tutorial')) return;
      localStorage.setItem('wandermere_tutorial', '1');
      scene.time.delayedCall(400, () => ui.openHowto());
    };
    if (!inv.party.length && !localStorage.getItem('wandermere_starter')) {
      ui.showStarterSelect((key) => {
        inv.starter(key);
        localStorage.setItem('wandermere_starter', '1');
        S.state.stats.species = inv.bestiaryCaught.size; scene._refreshQuest();
    ...
    window.WANDERMERE = { scene, player, creatures, inv, sky, build, ui };
    ...
    for (let i = balls.length - 1; i >= 0; i--) {
      ...
      if (b.phase === 'fly') {
        ...
        if (b.success) {
          ...
          const wasNew = !inv.bestiaryCaught.has(c.key);
          ...
          if (wasNew) { S.state.stats.species = inv.bestiaryCaught.size; scene._bumpStat('species', 0); scene._bestiaryMilestone(); }
...
// since generated frames can have slightly different amounts of padding).
...
function makeCreature(scene, key, level, x, y, sea, boss) {
  ...
  const texKey = ensureCreatureTexture(scene, key, sp);
  ...
  const spr = scene.add.image(0, REST, texKey);
  spr.setScale(base).setAlpha(1);
  spr.setOrigin(0.5, footFrac(scene, texKey));
