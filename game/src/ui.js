...
// panels (inventory, crafting, bestiary, party, shop, how-to-play,
// starter select). All icons are hand-drawn (SVG / colour chips)
// — no emoji. DOM lives in index.html + here.
// ============================================================

import { ITEMS, RECIPES, SPECIES, SHOP_OFFERS, SELL_PRICES, GOLD_SHOP, ACHIEVEMENTS, BUFFS, TYPE_COLORS, UPGRADES, ASCENSION } from './data.js?v=51';
import { creatureImageURL } from './sprites.js?v=51';
import { SFX } from './sfx.js?v=51';

const $ = (id) => document.getElementById(id);
const PANELS = ['inventory', 'bestiary', 'party', 'craft', 'shop', 'ach', 'upgrades', 'daily'];  // howto is a non-blocking sidebar
...
export class UI {
  ...
  _loadHotbar() {
    ...
    try {
      const a = JSON.parse(localStorage.getItem('wandermere_hotbar') || 'null');
      if (Array.isArray(a)) { const h = a.slice(0, 6).map((k) => (k && ITEMS[k]) ? k : null); while (h.length < 6) h.push(null); return h; }
    } catch (e) {}
    return def;
  }
  _saveHotbar() { try { localStorage.setItem('wandermere_hotbar', JSON.stringify(this.hotbar)); } catch (e) {} }
  toggleHotbar(item) {
  ...
  updatePartyDock() {
    ...
    list.innerHTML = active.map((m, i) => {
      ...
      return `<div class="dockrow"><img src="${creatureImageURL(m.key, sp, m.shiny)}" alt="">
  ...
  renderCraft() {
    ...
    const caught = this.inv.bestiaryCaught.size;
  ...
  renderParty() {
    ...
    this.inv.party.forEach((m, i) => {
      ...
      cell.innerHTML = `${isLead ? '<div class="leadtag">LEAD</div>' : (m.out ? '<div class="outtag">OUT</div>' : '')}
        <img src="${creatureImageURL(m.key, sp, m.shiny)}" alt="">
  ...
  _afterTeam() { this.renderParty(); if (this.openPanel === 'bestiary') this.renderBestiary(); this.updatePartyDock(); this.onTeamChange && this.onTeamChange(); }
  renderBestiary() {
    const grid = $('bestiary-grid'); grid.innerHTML = '';
    const all = Object.entries(SPECIES).sort((a, b) => a[1].no - b[1].no);
    $('bestiary-count').textContent = `${this.inv.bestiaryCaught.size} / ${all.length} caught`;
    for (const [key, sp] of all) {
      const caught = this.inv.bestiaryCaught.has(key), seen = this.inv.bestiarySeen.has(key);
      ...
      cell.innerHTML = (caught || seen
        ? `<img src="${creatureImageURL(key, sp, false)}" style="${caught ? '' : 'filter:brightness(0)'}" alt=""><span>${caught ? sp.name : '???'}</span>`
        : `<div class="dq">#${String(sp.no).padStart(3, '0')}</div><span>???</span>`)
  ...
  renderAch() {
    ...
    const cur = (stat) => stat === 'species' ? this.inv.bestiaryCaught.size : stat === 'plevel' ? this.inv.plevel : stat === 'goldEarned' ? this.inv.goldEarned : (this.inv.ach[stat] || 0);
  ...
  renderUpgrades() {
    ...
    const cost = m.ascensionCost(), eligible = m.canAscend(this.inv.bestiaryCaught.size), canAsc = eligible && this.inv.gold >= cost;
    asc.innerHTML = `<div class="asctitle">✦ ASCENSION — Gloam tier ${m.ascension}</div>
      <div class="ascdesc">Permanently +${Math.round(ASCENSION.xpPer * 100)}% XP, +${Math.round(ASCENSION.goldPer * 100)}% gold, +${Math.round(ASCENSION.shinyPer * 100)}% shiny odds and rarer spawns — stacking every tier. Keeps your bestiary, team and upgrades.</div>
  ...
  showStarterSelect(cb) {
    ...
    for (const key of ['vulmis', 'sprigling', 'aqulet']) {
      const sp = SPECIES[key];
      const card = document.createElement('button');
      card.className = 'starter-card';
      card.innerHTML = `<img src="${creatureImageURL(key, sp, false)}" alt=""><div class="sname">${sp.name}</div><div class="stypes">${sp.types.map((t) => typeChip(t) + ' ' + t).join(' ')}</div>`;
